...
feedback_dir = [pwd '/Data/NeuroFeedback_RunTwo'];
feedback_file_name = 'NeuroFeedback_RunTwo';
run_no = 2;

%% Optional to change
block_dur_TR = 20; % in TRs - at least 20
...
% Draws feedback for specified duration (num_trs)
...
%OUTPUTS
%bo - onset time of block (in s)
%be - end time of block (in s)
%bdur - duration of block (in s)
%block_start_tr - onset of cue on screen (in TR)
%block_vals - fMRI data from block (for each TR)
...
curr_feedback = (round((current_psc/PSC_thresh)*20)); %only deactivation
