...
%ROI PSC with confound - continuous image
...
sca;
clear all
global TR window scr_rect centreX centreY escapeKey start_time current_TBV_tr ROI_PSC ROI_vals PSC_thresh port_issue texture texture2;
...
feedback_dir = [pwd '\Data\NeuroFeedback_'];
feedback_file_name = 'NeuroFeedback_';
run_no = 

%Loading image
img = imread([pwd '\Data\relaxing_image.jpeg']);
%img2 = imread([pwd '\Data\image2.jpeg']);
...
try
    ...
    % Activating for changing opacity of image
    Screen('BlendFunction', window, 'GL_SRC_ALPHA', 'GL_ONE_MINUS_SRC_ALPHA');
    
    % Getting image representation
    texture = Screen('MakeTexture', window, img);
    %texture2 =  Screen('MakeTexture', window, img2); 
    ...
    Text = 'Brightness of the image will increase with better focus.';
...
function feedback_num = DrawFeedbackImage()
% Updates feedback screen based on latest score

%OUTPUT
%feedback_num - feedback value (between 0.1 and 1)

global window texture texture2

%rect_num is the number of rectangles to color corresponding to the current feedback value
feedback_num = calculate_feedback();

%Changing the image opacity based on current feedback value    
%Screen(‘DrawTexture’, windowPointer, texturePointer [,sourceRect] [,destinationRect] [,rotationAngle] [, filterMode] [, globalAlpha] [, modulateColor]);
Screen('DrawTexture',window,texture,[],[],[],[],feedback_num);
...
curr_feedback = round((current_psc/PSC_thresh),2); %only deactivation
