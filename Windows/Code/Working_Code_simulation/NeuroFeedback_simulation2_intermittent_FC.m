...
feedback_dir = [pwd '\Data\NeuroFeedback_RunThree'];
feedback_file_name = 'NeuroFeedback_RunThree';
run_no = 3;
...
if first_tr < current_TBV_tr %TBV should have surpassed the first actual TR by now
    ...
    [FC_val,~] = corr(med_vals_roi1,med_vals_roi2);
    
    %Feedback
    %Higher feedback value for higher FC value 
    FC_val_nonneg = FC_val + 1; %-1 to 1 becomes 0 to 2
    curr_feedback = (round(FC_val_nonneg,1)*10); 
